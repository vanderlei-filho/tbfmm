#ifndef TBFSMSTARPUCALLBACKSMPI_HPP
#define TBFSMSTARPUCALLBACKSMPI_HPP

#include <starpu.h>
#include <starpu_mpi.h>

class TbfSmStarpuCallbacksMPI {
public:
  template <class ThisClass, class CellContainerClass,
            class ParticleContainerClass>
  static void P2MCallback(void *buffers[], void *cl_arg) {
    ...
    starpu_codelet_unpack_args(cl_arg, &thisptr, &groupCellsData,
                               &groupCellsDataSize, &groupParticlesData,
                               &groupParticlesDataSize);
    ...
    unsigned char *leafMultipole =
        (unsigned char *)STARPU_VARIABLE_GET_PTR(buffers[2]);
    size_t leafMultipoleSize = STARPU_VARIABLE_GET_ELEMSIZE(buffers[2]);

    CellContainerClass leafGroupObj(leafData, leafDataSize, leafMultipole,
                                    leafMultipoleSize, nullptr, 0);
    const ParticleContainerClass particleGroupObj(particleData,
                                                  particleDataSize, nullptr, 0);

    thisptr->kernelWrapper.P2M(thisptr->kernels[starpu_worker_get_id()],
                               particleGroupObj, leafGroupObj);
  }
  ...
  static void P2PBetweenLeavesCallback(void *buffers[], void *cl_arg) {
    ...
    starpu_codelet_unpack_args(cl_arg, &thisptr, &indexesForGroup_first,
                               &srcDataCpu, &srcDataSizeCpu, &tgtDataCpu,
                               &tgtDataSizeCpu);
    ...
    unsigned char *tgtRhs =
        (unsigned char *)STARPU_VARIABLE_GET_PTR(buffers[3]);
    size_t tgtRhsSize = STARPU_VARIABLE_GET_ELEMSIZE(buffers[3]);

    ParticleContainerClass groupSrc(srcData, srcDataSize, srcRhs, srcRhsSize);
    ParticleContainerClass groupTarget(tgtData, tgtDataSize, tgtRhs,
                                       tgtRhsSize);

    thisptr->kernelWrapper.P2PBetweenGroups(
        thisptr->kernels[starpu_worker_get_id()], groupSrc, groupTarget,
  ...
  static void P2POneLeafCallback(void *buffers[], void *cl_arg) {
    ...
    starpu_codelet_unpack_args(cl_arg, &thisptr, &indexesForGroup_first,
                               &particleDataCpu, &particleDataSizeCpu);
    ...
    thisptr->kernelWrapper.P2PInGroup(thisptr->kernels[starpu_worker_get_id()],
                                      currentGroup, *indexesForGroup_first);
    thisptr->kernelWrapper.P2PInner(thisptr->kernels[starpu_worker_get_id()],
                                    currentGroup);
  }
  ...
  static void M2MCallback(void *buffers[], void *cl_arg) {
    ...
    unsigned char *lowerData =
        (unsigned char *)STARPU_VARIABLE_GET_PTR(buffers[0]);
    size_t lowerDataSize = STARPU_VARIABLE_GET_ELEMSIZE(buffers[0]);

    unsigned char *lowerMultipole =
        (unsigned char *)STARPU_VARIABLE_GET_PTR(buffers[1]);
    size_t lowerMultipoleSize = STARPU_VARIABLE_GET_ELEMSIZE(buffers[1]);

    unsigned char *upperData =
        (unsigned char *)STARPU_VARIABLE_GET_PTR(buffers[2]);
    size_t upperDataSize = STARPU_VARIABLE_GET_ELEMSIZE(buffers[2]);

    unsigned char *upperMultipole =
        (unsigned char *)STARPU_VARIABLE_GET_PTR(buffers[3]);
    size_t upperMultipoleSize = STARPU_VARIABLE_GET_ELEMSIZE(buffers[3]);

    const CellContainerClass lowerGroupObj(lowerData, lowerDataSize,
                                           lowerMultipole, lowerMultipoleSize,
                                           nullptr, 0);
    CellContainerClass upperGroupObj(upperData, upperDataSize, upperMultipole,
                                     upperMultipoleSize, nullptr, 0);

    thisptr->kernelWrapper.M2M(idxLevel,
                               thisptr->kernels[starpu_worker_get_id()],
                               lowerGroupObj, upperGroupObj);
  }
  ...
  static void M2LCallback(void *buffers[], void *cl_arg) {
    ...
    starpu_codelet_unpack_args(
        cl_arg, &thisptr, &idxLevel, &indexesForGroup_first, &groupCellsSrcData,
        &groupCellsSrcDataSize, &groupCellsTgtData, &groupCellsTgtDataSize);

    unsigned char *srcData =
        (unsigned char *)STARPU_VARIABLE_GET_PTR(buffers[0]);
    size_t srcDataSize = STARPU_VARIABLE_GET_ELEMSIZE(buffers[0]);

    unsigned char *srcMultipole =
        (unsigned char *)STARPU_VARIABLE_GET_PTR(buffers[1]);
    ...
    unsigned char *tgtLocal =
        (unsigned char *)STARPU_VARIABLE_GET_PTR(buffers[3]);
    size_t tgtLocalSize = STARPU_VARIABLE_GET_ELEMSIZE(buffers[3]);

    const CellContainerClass groupSrc(srcData, srcDataSize, srcMultipole,
                                      srcMultipoleSize, nullptr, 0);
    CellContainerClass groupTarget(tgtData, tgtDataSize, nullptr, 0, tgtLocal,
                                   tgtLocalSize);

    thisptr->kernelWrapper.M2LBetweenGroups(
        idxLevel, thisptr->kernels[starpu_worker_get_id()], groupTarget,
        groupSrc, *indexesForGroup_first);
  }
  ...
  static void M2LInnerCallback(void *buffers[], void *cl_arg) {
    ...
    size_t groupCellsDataSize;
    starpu_codelet_unpack_args(cl_arg, &thisptr, &idxLevel,
                               &indexesForGroup_first, &groupCellsData,
                               &groupCellsDataSize);
    ...
    CellContainerClass currentGroup(srcData, srcDataSize, srcMultipole,
                                    srcMultipoleSize, srcLocal, srcLocalSize);

    thisptr->kernelWrapper.M2LInGroup(idxLevel,
                                      thisptr->kernels[starpu_worker_get_id()],
                                      currentGroup, *indexesForGroup_first);
  }
  ...
  static void L2LCallback(void *buffers[], void *cl_arg) {
    ...
    unsigned char *lowerLocal =
        (unsigned char *)STARPU_VARIABLE_GET_PTR(buffers[3]);
    size_t lowerLocalSize = STARPU_VARIABLE_GET_ELEMSIZE(buffers[3]);

    const CellContainerClass upperGroupObj(upperData, upperDataSize, nullptr, 0,
                                           upperLocal, upperLocalSize);
    CellContainerClass lowerGroupObj(lowerData, lowerDataSize, nullptr, 0,
                                     lowerLocal, lowerLocalSize);

    thisptr->kernelWrapper.L2L(idxLevel,
                               thisptr->kernels[starpu_worker_get_id()],
                               upperGroupObj, lowerGroupObj);
  }
  ...
  static void L2PCallback(void *buffers[], void *cl_arg) {
    ...
    const CellContainerClass leafGroupObj(leafData, leafDataSize, nullptr, 0,
                                          leafLocal, leafLocalSize);
    ParticleContainerClass particleGroupObj(particleData, particleDataSize,
                                            particleRhs, particleRhsSize);

    thisptr->kernelWrapper.L2P(thisptr->kernels[starpu_worker_get_id()],
                               leafGroupObj, particleGroupObj);
  }
};
