 ...
 * P. 6594, Eq. 1
 */
template <class RealType_T, int P,
...
private:
  ...
  std::complex<RealType>
      rotationExpMinusImPhi[8][SizeArray]; //< This is the vector use for the
                                           //rotation around z for the M2M
                                           //(multipole)
  std::complex<RealType>
  ...
  RealType DlmkCoefOMinusTheta[8][SizeDlmkMatrix]; //< d_lmk for Multipole
                                                   //reverse rotation

  RealType DlmkCoefMTheta[8][SizeDlmkMatrix];      //< d_lmk for Local rotation
  RealType DlmkCoefMMinusTheta[8][SizeDlmkMatrix]; //< d_lmk for Local reverse
                                                   //rotation

  RealType DlmkCoefM2LOTheta[343]
                            [SizeDlmkMatrix]; //< d_lmk for Multipole rotation
  RealType DlmkCoefM2LMMinusTheta[343][SizeDlmkMatrix]; //< d_lmk for Local
                                                        //reverse rotation
          ...
          for (int idxY = -3; idxY <= 3; ++idxY) {
            for (int idxZ = -3; idxZ <= 3; ++idxZ) {
              ...
              if (idxX < -1 || 1 < idxX || idxY < -1 || 1 < idxY || idxZ < -1 ||
                ...
                const std::array<RealType, 3> relativePosition{
                    {-RealType(idxX) * boxWidthAtLevel,
                ...
                const RealType b =
                    std::sqrt((relativePosition[0] * relativePosition[0]) +
   ...
   * \beta ) }
   * \f]
   * Because we store only P_lm for l >= 0 and m >= 0 we use the relation of
   * symetrie as:
  ...
  void precomputeRotationVectors() {
    ...
    for (int l = 0; l <= P; ++l) {
      for (int m = 0; m <= l; ++m) {
        sqrtDoubleFactorials[l][m] =
            std::sqrt(factorials[l - m] * factorials[l + m]);
      }
    }
    ...
    for (int x = 1; x <= 3; ++x) {
      for (int y = 0; y <= x; ++y) {
        for (int z = 1; z <= 3; ++z) {
          const RealType inclinaison =
              FSpherical<RealType>(std::array<RealType, 3>{
    ...
    for (int idxChild = 0; idxChild < 8; ++idxChild) {
      ...
      // compute the last part with l == P
      {
        ...
        for (int m = 0; m <= P; ++m, ++index_lm) {
          ...
          // \beta )
          rotationExpMinusImPhi[idxChild][index_lm] =
              std::complex<RealType>(std::cos(-mphi), std::sin(-mphi));
          // M_{l,m}( \alpha, \beta + \phi ) = e^{i \phi m} M_{l,m}( \alpha,
          // \beta )
          rotationExpImPhi[idxChild][index_lm] =
              std::complex<RealType>(std::cos(mphi), std::sin(mphi));
        }
      }
      // Then for l < P it just a copy of the previous computed vector
      {
        ...
        for (int l = 0; l < P; ++l) {
          ...
          FMemUtils::copyall(&rotationExpImPhi[idxChild][0] + index_lm,
                             &rotationExpImPhi[idxChild][0] + index_P0, l + 1);
      ...
      { // Then compute the inclinaison rotation
        ...
        for (int l = 0; l <= P; ++l) {
          for (int m = 0; m <= l; ++m) {
            ...
            for (int k = 1; k <= l; ++k) {
              const RealType d_lm_minus_k =
                  dlmkMatrix[dx + 3][dy][dz + 3][l][m + P][-k + P];
              const RealType d_lmk =
                  dlmkMatrix[dx + 3][dy][dz + 3][l][m + P][k + P];
              const RealType d_lm_minus_k_minusTheta =
                  dlmkMatrix[-dx + 3][dy][dz + 3][l][m + P][-k + P];
              const RealType d_lmk_minusTheta =
                  dlmkMatrix[-dx + 3][dy][dz + 3][l][m + P][k + P];
      ...
      for (int idxY = -3; idxY <= 3; ++idxY) {
        for (int idxZ = -3; idxZ <= 3; ++idxZ) {
          ...
          if (idxX < -1 || 1 < idxX || idxY < -1 || 1 < idxY || idxZ < -1 ||
            ...
            const std::array<RealType, 3> relativePosition{
                {-RealType(idxX) * boxWidth, -RealType(idxY) * boxWidth,
            ...
            // first compute the last part with l == P
            {
              ...
              for (int m = 0; m <= P; ++m, ++index_lm) {
                const RealType mphi =
                    (sph.getPhiZero2Pi() + PIDiv2) * RealType(m);
                ...
                // \alpha, \beta )
                rotationM2LExpImPhi[position][index_lm] =
                    std::complex<RealType>(std::cos(mphi), std::sin(mphi));
              }
            }
   ...
   * \f]
   */
  void DlmkZNegative(RealType dlmk[P + 1][P2 + 1][P2 + 1],
                     const RealType dlmkZPositif[P + 1][P2 + 1][P2 + 1]) const {
    for (int l = 0; l <= P; ++l) {
      for (int m = -l; m <= l; ++m) {
        // if l+m is odd
        if ((l + m) & 0x1) {
   ...
   * \f]
   */
  void DlmkInverseZNegative(
      RealType dlmk[P + 1][P2 + 1][P2 + 1],
      const RealType dlmkZPositif[P + 1][P2 + 1][P2 + 1]) const {
    for (int l = 0; l <= P; ++l) {
      for (int m = -l; m <= l; ++m) {
        if ((l + m) & 0x1) {
   ...
   * \f]
   */
  void DlmkInverse(RealType dlmk[P + 1][P2 + 1][P2 + 1],
                   const RealType dlmkZPositif[P + 1][P2 + 1][P2 + 1]) const {
    for (int l = 0; l <= P; ++l) {
      for (int m = -l; m <= l; ++m) {
  ...
  void DlmkBuild(RealType dlmk[P + 1][P2 + 1][P2 + 1],
                 const RealType inTheta) const {
      ...
      // g{l,m} = sqrt( (l - m + 1) / (l+m)) g{l,m-1}  for l > 0, 0 < m <= l
      {
        ...
        for (int l = 1; l <= P; ++l, ++fl) {
          ...
          for (int m = 1; m <= l; ++m, ++index_lm, ++fm) {
            g[index_lm] =
                std::sqrt((fl - fm + F1) / (fl + fm)) * g[index_lm - 1];
          }
          ++index_lm;
        }
      }
      ...
      for (int l = 1; l <= P; ++l, ++fl) {
        ...
        for (int k = l; k > -l; --k, --fk) {
          ...
          for (int m = 0; m < l; ++m, ++fm) {
            dlmk[l][P + m][P + k - 1] =
                (std::sqrt((fl * (fl + F1) - fm * (fm + F1)) /
                           (fl * (fl + F1) - fk * (fk - F1))) *
                 dlmk[l][P + m + 1][P + k]) +
                ((fm + fk) * sinTheta * dlmk[l][P + m][P + k] /
                 (std::sqrt(fl * (fl + F1) - fk * (fk - F1)) *
                  (F1 + cosTheta)));
          }
          // Equ 26
          ...
          dlmk[l][P + l][P + k - 1] =
              ...
              (std::sqrt(fl * (fl + F1) - fk * (fk - F1)) * (F1 + cosTheta));
        }
        // Equ 27
   ...
   * multipole method Formula number (22)
   * \f[
   * P_{0,0} = 1
   ...
   * ,l \ge 1, 0 \leq m \le l-1
   * \f]
   */
  void computeLegendre(RealType legendre[], const RealType inCosTheta,
                       const RealType inSinTheta) const {
   ...
   */
  static void RotationYWithDlmk(std::complex<RealType> vec[],
                                const RealType *dlmkCoef) {
    RealType originalVec[2 * SizeArray];
    FMemUtils::copyall(originalVec, reinterpret_cast<const RealType *>(vec),
                       2 * SizeArray);
   ...
   */
  static void RotationZVectorsMul(std::complex<RealType> *dest,
                                  const std::complex<RealType> *src,
                                  const int inSize = SizeArray) {
    const std::complex<RealType> *const lastElement = dest + inSize;
    const std::complex<RealType> *const intermediateLastElement =
        dest + (inSize & ~0x3);
   ...
   */
  std::array<RealType, 3>
  getLeafCenter(const std::array<long int, 3> &coordinate) const {
    return std::array<RealType, 3>{
        boxCorner[0] +
            (RealType(coordinate[0]) + RealType(.5)) * widthAtLeafLevel,
        boxCorner[1] +
            (RealType(coordinate[1]) + RealType(.5)) * widthAtLeafLevel,
        boxCorner[2] +
            (RealType(coordinate[2]) + RealType(.5)) * widthAtLeafLevel};
  }

  std::array<RealType, 3>
  getLeafCenter(const typename SpaceIndexType::IndexType &inIndex) const {
...
public:
   ...
   * \f[
   * \omega (q,a) = q \frac{a^{l}}{(l+|m|)!} P_{lm}(cos( \alpha ) )e^{-im
   * \beta},
   * \f]
   ...
   *
   * Finally, the |r-a|^{-1} expansion is a contraction of O{l,m}(a) with
   * M{l,m}{r}, where M{l,m}(r) are chargeless Taylor counterparts of the
   * O{l,m}(a). Assumption: |a| < |r|.
   */
  ...
  void P2M(const CellSymbolicData &LeafIndex,
           ...
           const unsigned long long inNbParticles, LeafClass &LeafCell) {
    ...
    const std::array<RealType, SpaceIndexType_T::Dim> cellPosition =
        getLeafCenter(LeafIndex.boxCoord);
    ...
    const RealType *const physicalValues =
        SourceParticles[3]; // pointer to contiguously allocated physical
  ...
  void M2M(const CellSymbolicData & /*inParentIndex*/, const long int inLevel,
           const CellClassContainer &inLowerCell, CellClass &inOutUpperCell,
    ...
    for (int idxChild = 0; idxChild < inNbChildren; ++idxChild) {
      ...
      // rotate it forward
      RotationZVectorsMul(source_w,
                          rotationExpMinusImPhi[childrenPos[idxChild]]);
   ...
   * O_{j,-m}(a) } , \mbox{\textrm{ j bounded by P-l } }
   * \f]
   * As describe in the paper, when need first to rotate the SH
   * then transfer using the formula
   * and finaly rotate back.
   */
  template <class CellSymbolicData, class CellClassContainer, class CellClass>
  void M2L(const CellSymbolicData & /*inTargetIndex*/, const long int inLevel,
    ...
    for (int idxNeigh = 0; idxNeigh < inNbNeighbors; ++idxNeigh) {
      const RealType *const coef =
          M2LTranslationCoef[inLevel][neighPos[idxNeigh]];
      // Copy multipole data into buffer
      FMemUtils::copyall(source_w, inInteractingCells[idxNeigh].get(),
                         SizeArray);

      // Rotate
      RotationZVectorsMul(source_w,
                          rotationM2LExpMinusImPhi[neighPos[idxNeigh]]);
  ...
  void L2L(const CellSymbolicData & /*inParentIndex*/, const long int inLevel,
           const CellClass &inUpperCell, CellClassContainer &inOutLowerCell,
   ...
   * \f]
   */
  template <class CellSymbolicData, class LeafClass, class ParticlesClass,
            class ParticlesClassRhs>
  void L2P(const CellSymbolicData &LeafIndex, const LeafClass &LeafCell,
           const long int /*particlesIndexes*/[],
           const ParticlesClass &inOutParticles,
           ParticlesClassRhs &inOutParticlesRhs, const long int inNbParticles) {
    const RealType i_pow_m[4] = {0, PIDiv2, PI, -PIDiv2};
    // Take the local value from the cell
    const std::complex<RealType> *const u = &LeafCell[0];

    // Copying the position is faster than using cell position
    const std::array<RealType, 3> cellPosition =
        getLeafCenter(LeafIndex.boxCoord);
    ...
    for (long int idxPart = 0; idxPart < inNbParticles; ++idxPart) {
      ...
      {
        for (int m = 0; m <= P; ++m) {
          const RealType m_phi_i_pow_m =
              RealType(m) * sph.getPhi() + i_pow_m[m & 0x3];
      ...
      // compute the forces
      {
        ...
        for (int l = 1; l <= P; ++l, ++fl) {
          ...
          {
            const RealType coef = minus_r_pow_l_div_fact_lm[index_lm] *
                                  (fl *
          ...
          for (int m = 1; m <= l; ++m, ++index_lm) {
            ...
            {
              const RealType legendre_l_minus_1 =
                  (m == l) ? RealType(0.0)
                           : RealType(l + m) * legendre[index_lm - l];
              const RealType coef =
                  minus_r_pow_l_div_fact_lm[index_lm] *
              ...
              // F(O) += 2 * Real(L dI/dO)
              FO += RealType(2.0) * (u[index_lm].real() * dI_real -
        ...
        const RealType forceY =
            (sinPhi * sph.getSinTheta() * Fr + sinPhi * sph.getCosTheta() * FO +
             cosPhi * Fp) *
            physicalValue;
      ...
      // compute the potential
      {
        ...
        for (int l = 0; l <= P; ++l) {
          ...
          for (int m = 1; m <= l; ++m, ++index_lm) {
            ...
            const RealType I_real = coef * cos_m_phi_i_pow_m[m];
            const RealType I_imag = coef * sin_m_phi_i_pow_m[m];
            magnitude += RealType(2.0) * (u[index_lm].real() * I_real -
  ...
  template <class LeafSymbolicData, class ParticlesClassValues,
            class ParticlesClassRhs>
  void
  P2P(const LeafSymbolicData &inNeighborIndex,
      const long int /*neighborsIndexes*/[],
      const ParticlesClassValues &inNeighbors,
      ParticlesClassRhs &inNeighborsRhs, const long int inNbParticlesNeighbors,
    ...
    if constexpr (SpaceIndexType::IsPeriodic) {
      using PeriodicShifter =
          typename TbfPeriodicShifter<RealType, SpaceIndexType>::Neighbor;
      if (PeriodicShifter::NeedToShift(inNeighborIndex, inTargetIndex,
                                       spaceIndexSystem, arrayIndexSrc)) {
        const auto duplicateSources =
            PeriodicShifter::DuplicatePositionsAndApplyShift(
                inNeighborIndex, inTargetIndex, spaceIndexSystem, arrayIndexSrc,
                inNeighbors, inNbParticlesNeighbors);
        FP2PR::template FullMutual<RealType>(
            (duplicateSources), (inNeighborsRhs), inNbParticlesNeighbors,
      ...
      FP2PR::template FullMutual<RealType>((inNeighbors), (inNeighborsRhs),
                                           inNbParticlesNeighbors, (inTargets),
  ...
  void P2PTsm(const LeafSymbolicDataSource &inNeighborIndex,
              ...
              const LeafSymbolicDataTarget &inTargetIndex,
      ...
      if (PeriodicShifter::NeedToShift(inNeighborIndex, inTargetIndex,
        ...
        const auto duplicateSources =
            PeriodicShifter::DuplicatePositionsAndApplyShift(
                inNeighborIndex, inTargetIndex, spaceIndexSystem, arrayIndexSrc,
                inNeighbors, inNbParticlesNeighbors);
        FP2PR::template GenericFullRemote<RealType>(
            (duplicateSources), inNbParticlesNeighbors, (inTargets),
            (inTargetsRhs), inNbOutParticles);
        PeriodicShifter::FreePositions(duplicateSources);
      } else {
        FP2PR::template GenericFullRemote<RealType>(
            (inNeighbors), inNbParticlesNeighbors, (inTargets), (inTargetsRhs),
            inNbOutParticles);
      }
    } else {
      FP2PR::template GenericFullRemote<RealType>(
  ...
  void P2PInner(const LeafSymbolicData & /*inIndex*/,
                const long int /*indexes*/[],
                const ParticlesClassValues &inTargets,
                ParticlesClassRhs &inTargetsRhs,
                const long int inNbOutParticles) const {
    FP2PR::template GenericInner<RealType>((inTargets), (inTargetsRhs),
                                           inNbOutParticles);
  }
  ...
  void initCudaKernelData(const cudaStream_t &inStream) {
    // Nothing to initialize here, we'll get the stream when needed
  }

  auto getCudaKernelData() {
    return CudaKernelData{starpu_cuda_get_local_stream()};
  }
  ...
  P2PTsmCuda(const CudaKernelData &cudaKernelData,
             const LeafSymbolicDataSource &inNeighborIndex,
             const long int /*neighborsIndexes*/[],
             const ParticlesClassValuesSource &inNeighbors,
             const long int inNbParticlesNeighbors,
             const LeafSymbolicDataTarget &inTargetIndex,
    ...
    TbfP2PCuda::template GenericFullRemote<RealType>(
        (inNeighbors), inNbParticlesNeighbors, (inTargets), (inTargetsRhs),
        inNbOutParticles, cudaKernelData.stream);
  }

  template <class LeafSymbolicData, class ParticlesClassValues,
            class ParticlesClassRhs>
  __device__ static void P2PInnerCuda(
      const CudaKernelData &cudaKernelData,
      const LeafSymbolicData & /*inIndex*/, const long int /*indexes*/[],
      const ParticlesClassValues &inTargets, ParticlesClassRhs &inTargetsRhs,
      const long int inNbOutParticles) /*const*/ {
    TbfP2PCuda::template GenericInner<RealType>(
        (inTargets), (inTargetsRhs), inNbOutParticles, cudaKernelData.stream);
  }
#endif
};
